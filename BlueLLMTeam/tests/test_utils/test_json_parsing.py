import pytest
from BlueLLMTeam.utils import extract_json_from_brackets

def test_valid_json_square_brackets():
    text = "Some text before [ { \"key1\": \"value1\" }, { \"key2\": \"value2\" } ] some text after"
    expected = [{"key1": "value1"}, {"key2": "value2"}]
    assert extract_json_from_brackets(text) == expected
...
def test_invalid_json():
    ...
    with pytest.raises(ValueError, match="Invalid JSON data"):
        extract_json_from_brackets(text)

def test_no_json_data():
    text = "Some text before and after with no JSON data"
    with pytest.raises(ValueError, match="No JSON data found inside square or curly brackets"):
        extract_json_from_brackets(text)
